...
makeDTM <- function(tx, dict=NULL) {
  ...
  c <- tm_map(c, removeWords, stopwords("italian"))
  c <- tm_map(c, stemDocument, language = "italian")
...
data = read.csv("imdb_small.csv", sep=",")
ID = seq.int(nrow(data))
QUESTIONS = as.character(data$review)
TYPE=data$label
...
train.set=sample(nrow(d),nrow(d)*0.7)
test.set = 1:dim(d)[1]
